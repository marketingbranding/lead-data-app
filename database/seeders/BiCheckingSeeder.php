...
class BiCheckingSeeder extends Seeder
{
    public function run(): void
    {
        $csv = Reader::createFromPath(database_path('../../references/pipelinekpr/1 - bi_checking.csv'));
        ...
        foreach ($csv->getRecords() as $record) {
            $records[] = [
                ...
                'tanggal_slik' => $this->parseDate($record['tanggal_slik']),
                'hasil_slik' => $this->parseNullable($record['hasil_slik']),
                'keterangan' => $this->parseNullable($record['keterangan']),
                'created_at' => now(),
                'updated_at' => now(),
            ];
            if (count($records) >= 100) {
                DB::table('bi_checking')->insert($records);
                $records = [];
            }
        }
        if (!empty($records)) {
