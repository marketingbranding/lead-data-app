...
class PpjbDevSeeder extends Seeder
{
    public function run(): void
    {
        $csv = Reader::createFromPath(database_path('../../references/pipelinekpr/5 - ppjb_dev.csv'));
        ...
        foreach ($csv->getRecords() as $record) {
            $records[] = [
                ...
                'tanggal_sp3k' => $this->parseDate($record['tanggal_sp3k']),
                'tanggal_ttd_ppjb' => $this->parseDate($record['tanggal_ttd_ppjb']),
            ...
            if (count($records) >= 100) {
                DB::table('ppjb_dev')->insert($records);
                $records = [];
            }
        }
        if (!empty($records)) {
